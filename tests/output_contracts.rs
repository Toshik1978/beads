...
// The output-contract goldens: what `br` promises to anything that parses
// its output. `common` is the `test-support` crate, and `extern crate` must
// sit at the crate root, which is this file.
...
pub(crate) fn init_workspace() -> BrWorkspace {
...
pub(crate) fn create_issue(workspace: &BrWorkspace, title: &str, label: &str) -> String {
...
static TMP_PATH_RE: LazyLock<Regex> = LazyLock::new(|| {
    // `BrWorkspace` (bds-04l.3.1) canonicalizes its root, so on macOS every
    // temp path now carries a `/private` prefix ahead of `/var/folders/...`.
    // The `/var/folders` alternative was previously unanchored against that
    // prefix and its character class excluded `.`, so it matched only the
    // `/var/folders/.../T/` middle segment: `/private` was left unconsumed
    // ahead of it, and the `.tmpXXXX` leaf was left unconsumed after it,
    // producing `/private/TMP.tmplqbQg8/` instead of the expected `/TMP/`.
    // Absorb an optional `/private` prefix and allow `.` in the character
    // class so the `.tmpXXXX` leaf is consumed too.
    Regex::new(
        r"(?:/private)?(?:/data)?/tmp/[A-Za-z0-9_-]*/?\.tmp[a-zA-Z0-9]+|(?:/private)?/var/folders/[A-Za-z0-9_-]+/[A-Za-z0-9_-]+/[A-Za-z0-9_-]+/\.tmp[a-zA-Z0-9]+|(?:/private)?/var/folders/[a-zA-Z0-9/_-]+",
    )
...
pub(crate) struct TextNormConfig {
...
pub(crate) fn normalize_minimal(output: &str) -> String {
...
#[allow(clippy::too_many_lines)]
pub(crate) fn normalize_json(json: &Value) -> Value {
    match json {
        Value::Object(map) => {
...
pub(crate) fn normalize_jsonl(contents: &str) -> String {
...
// `#[path]` because this file is the crate root: a bare `mod json_output;`
// would resolve to `tests/json_output.rs` and claim a sibling test binary's
// name. Keeping the submodules in `tests/output_contracts/` is also what puts
// insta's goldens in `tests/output_contracts/snapshots/`, since insta writes
// beside the source file that asserts them.
#[path = "output_contracts/history_diff_output.rs"]
mod history_diff_output;
#[path = "output_contracts/json_output.rs"]
mod json_output;
