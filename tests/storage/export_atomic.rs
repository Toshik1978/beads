...
fn concurrent_exports_no_corruption() {
    ...
    // The pure-Rust engine's `Connection` was `!Send` (it held an `Rc`), so the
    // handle could not cross a thread boundary. `rusqlite`'s is `Send`, so that
    // constraint is gone; the shape is kept because what this test is about is
    // four independent exports to four paths, not handle sharing.
