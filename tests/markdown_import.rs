...
epic
### Agent Context
{"skills": ["porting-to-rust"], "constraints": ["no unsafe code; std only"]}
...
";
    ...
    // Upstream #368: an unresolved declared dependency edge is still a warning
    // (the issue is created, only the edge is dropped), but the command now
    // exits with the DependencyNotFound code so scripted callers see that the
    // persisted graph differs from the file rather than a false success.
    assert_eq!(
        output.status.code(),
        Some(5),
        "dropped dependency edge should exit DependencyNotFound: stdout={} stderr={}",
        output.stdout,
        output.stderr
    );
    assert!(
        ...
        "expected warning for missing issue id, got: {}",
        output.stderr
    );
}
...
";
    ...
    assert!(
        output.stdout.contains("failed to create any issues from"),
