...
#[test]
fn test_create_file_empty_markdown_emits_empty_json_array() {
    let temp = tempfile::tempdir().unwrap();
    let path = temp.path();
    ...
    let output = Command::new(bin.as_os_str())
        ...
        .arg("--file")
        .arg("empty.md")
        .env("BR_OUTPUT_FORMAT", "json")
        .output()
        .expect("create --file with empty markdown in json mode");
    ...
    let decoded_json: serde_json::Value = serde_json::from_str(stdout.trim()).expect("valid JSON");

    assert_eq!(
        decoded_json,
        serde_json::Value::Array(Vec::new()),
        "empty markdown imports should emit an empty structured payload in JSON mode"
    );
}
