...
use beads::storage::conn::DbError;
...
fn assert_fresh_lookup_round_trip(workspace: &BrWorkspace, alt_db: &Path, i: usize) {
    ...
    assert_eq!(
        ...
        Some("raw"),
        "raw config lookup should keep returning the configured prefix after rebuild on loop {i}"
    );
...
#[test]
fn e2e_raw_sqlite_keyed_lookup_matches_full_scan_after_alt_rebuild() {
    const LOOP_COUNT: usize = 20;

    let _log = common::test_log("e2e_raw_sqlite_keyed_lookup_matches_full_scan_after_alt_rebuild");
