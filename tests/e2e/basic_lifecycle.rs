...
fn assert_issue_description(workspace: &BrWorkspace, issue_id: &str, expected: &str) {
    ...
    // The storage layer treats an empty text column as absent (`get_non_empty_str`
    // in src/storage/sqlite.rs), so a cleared description round-trips as JSON
    // `null`, not `""`. Normalize before comparing; a non-empty `expected` still
    // has to match exactly.
    let actual = issues[0]["description"].as_str().unwrap_or("");
...
fn e2e_basic_lifecycle() {
    ...
    let close_args = vec![
        ...
        id,
        "--reason".to_string(),
        "lifecycle smoke complete".to_string(),
    ];
