...
fn regression_full_cli_does_not_touch_git() {
    ...
    // create --silent (ID-only output)
    let (git_snap, head, count, silent_result) = check_git_safety!(
        &workspace,
        git_snap,
        head,
        count,
        ["create", "Quick issue", "--silent", "--no-auto-flush"],
        "create_silent"
    );
    let id3 = silent_result.stdout.trim().to_string();
