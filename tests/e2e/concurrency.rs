...
fn e2e_parallel_read_only_commands_serialize_without_busy_on_drop() {
    ...
    let isolated =
        IsolatedDataset::from_dataset(KnownDataset::ThisRepo).expect("copy beads dataset");
...
/// Test that actor-aware command families like claim can interleave
    ...
    let commenter = {
        ...
        thread::spawn(move || {
            ...
            for i in 0..8 {
                ...
                // claimer thread updates the JSONL but leaves dirty flags
...
fn e2e_parallel_mixed_db_commands_preserve_sqlite_integrity() {
    ...
    assert_upstream_sqlite_integrity_ok(&root, "after repeated status reads");
}
