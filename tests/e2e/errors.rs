...
fn e2e_sync_rename_prefix_failed_import_restores_original_corrupt_db_family() {
    ...
    // `--json` writes the structured error envelope to stdout (upstream #336);
    // stderr carries only the recovery WARN lines.
    let import_error = parse_error_json(&result.stdout).expect("structured import error");
    assert!(
...
/// Build the fixture for `e2e_transition_required_fields_are_structured_fresh_and_atomic`:
/// an initialized workspace holding one claimed issue that already carries a
/// historical comment, under a policy requiring both `acceptance_criteria` and
/// `transition_comment` on `in_progress -> in_review`.
///
/// Returns the workspace (whose `TempDir` must outlive the assertions) and the
/// created issue's id.
///
/// Extracted purely so the test below stays under clippy's `too_many_lines`
/// limit without an `#[allow]`. The assertion sequence that remains is one
/// continuous scenario — each step's meaning depends on the state the previous
/// one left behind — and is deliberately not split any further.
fn required_fields_fixture() -> (BrWorkspace, String) {
    let workspace = BrWorkspace::new();
...
fn e2e_transition_required_fields_are_structured_fresh_and_atomic() {
    ...
    let (workspace, id) = required_fields_fixture();

    // Acceptance-criteria values are markdown checklists, so they start with
    // `- `. clap has no `allow_hyphen_values` on `--acceptance-criteria`, so the
    // separate-argument spelling is parsed as a short-flag cluster and rejected
    // with a usage error (exit 2) before the policy engine ever runs. Use the
    // `--flag=value` spelling, which is unambiguous.
    //
    // bds-04l.12 settled that this stays the supported spelling -- see
    // `hyphen_leading_value_error_names_the_flag` below for why, and for the
    // diagnosis the separate-argument form now gets.
    let missing_comment = run_br(
        ...
        [
            ...
            "--status",
            "in_review",
            "--acceptance-criteria=- [x] Exercised",
            "--json",
        ],
...
fn e2e_structured_error_conflict_markers() {
    ...
    // `--json` writes the structured error envelope to stdout, not stderr
    // (upstream #336), so the conflict-marker diagnosis is on stdout.
    let error = parse_error_json(&result.stdout).expect("structured conflict-marker error");
    assert!(verify_error_structure(&error), "missing required fields");
...
fn e2e_sync_flush_refuses_to_overwrite_conflict_markers() {
    ...
    // `--json` routes the structured error envelope to stdout (upstream #336).
    let refusal_error =
        parse_error_json(&refused_flush.stdout).expect("structured flush-refusal error");
    assert!(
        verify_error_structure(&refusal_error),
        "missing required fields"
    );
    let refusal_message = refusal_error["error"]["message"]
        .as_str()
        .unwrap_or_default();
    assert!(
        refusal_message.to_ascii_lowercase().contains("conflict"),
...
///
/// Parsing is deliberately unchanged: `allow_hyphen_values` would make
/// `--acceptance-criteria --json` swallow `--json` as the value, trading a loud
/// error for a quiet wrong one. What is asserted here is that the failure now
/// explains itself.
#[test]
fn hyphen_leading_value_error_names_the_flag() {
    let _log = common::test_log("hyphen_leading_value_error_names_the_flag");
    let workspace = BrWorkspace::new();
    assert!(run_br(&workspace, ["init"], "init").status.success());
    let create = run_br(&workspace, ["create", "AC probe"], "create");
    assert!(create.status.success(), "create failed: {}", create.stderr);
    let id = parse_created_id(&create.stdout);

    let rejected = run_br(
        &workspace,
        ["update", &id, "--acceptance-criteria", "- [x] Exercised"],
        "ac_separate_argument",
    );

    assert_eq!(
        rejected.status.code(),
        Some(2),
        "a clap usage error, not a br error: {}",
        rejected.stderr
    );
    assert!(
        rejected.stderr.contains("--acceptance-criteria"),
        "the diagnosis must name the flag clap's own message omits, got: {}",
        rejected.stderr
    );
    assert!(
        rejected
            .stderr
            .contains("--acceptance-criteria=- [x] Exercised"),
        "the diagnosis must spell out the working form, got: {}",
        rejected.stderr
    );

    // The suggested spelling has to actually work, or the hint sends the
    // operator somewhere no better.
    let accepted = run_br(
        &workspace,
        [
            "update",
    ...
    assert!(
        accepted.status.success(),
        "the spelling the hint suggests must succeed: {}",
        accepted.stderr
    );

    let show = run_br(&workspace, ["show", &id, "--json"], "show_ac");
    let issues: Value =
        serde_json::from_str(&extract_json_payload(&show.stdout)).expect("show json");
    assert_eq!(
        issues[0]["acceptance_criteria"], "- [x] Exercised",
