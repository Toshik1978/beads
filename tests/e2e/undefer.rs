...
fn test_soft_defer_behavior() {
    ...
    // Try undefer (via `update --defer ""`; the surviving invocation for
    // clearing a soft defer, per bds-04l.2.7's corrected mapping table)
    let undefer = run_br(&workspace, ["update", id, "--defer", ""], "undefer");
...
fn test_soft_defer_preserves_in_progress_status() {
    ...
    // No --status here: omitting it is exactly what preserves the
    // in_progress status this test asserts on (soft undefer, per the
    // brief's row 5: "Undefer preserves a soft defer's status").
    let undefer = run_br(
        &workspace,
        ["update", id, "--defer", "", "--json"],
        "undefer",
    );
    assert!(
