...
#[test]
fn e2e_quiet_flag_config_epic_and_label_subcommands() {
    let _log = common::test_log("e2e_quiet_flag_config_epic_and_label_subcommands");
    let workspace = BrWorkspace::new();
...
#[test]
fn e2e_quiet_flag_stale_and_info_subcommands() {
    let _log = common::test_log("e2e_quiet_flag_stale_and_info_subcommands");
    let workspace = BrWorkspace::new();
    ...
    let create = run_br(
        &workspace,
        ["create", "Quiet stale/info test"],
        "create_stale_info_quiet",
    );
    assert!(create.status.success(), "create failed: {}", create.stderr);
