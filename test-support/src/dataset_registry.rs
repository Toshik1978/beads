...
///
/// This previously listed three more variants pointing at benchmark dataset
/// roots on the original author's machine, under an absolute path outside
/// this repo. Those paths are unreachable on any other machine and nothing
/// outside this file ever named the variants directly — every consumer
/// either uses `ThisRepo` explicitly or loops over `all()` and skips whatever
/// `beads_dir()` reports as absent — so they were dropped rather than
...
impl KnownDataset {
    ...
    pub fn source_path(self) -> PathBuf {
        match self {
            // Use CARGO_MANIFEST_DIR for ThisRepo since we're running from within the repo
            Self::ThisRepo => crate::repo_root(),
        }
    }
...
fn workspace_failure_fixture_root() -> PathBuf {
    crate::repo_root().join(WORKSPACE_FAILURE_FIXTURE_DIR)
}
...
pub fn isolated_workspace_failure_fixture(
    ...
    // Canonicalize for the same reason `BrWorkspace::new` does: `br` resolves
    // every path it reports (BEADS_DIR goes through
    // `config::routing::follow_redirects`), so a raw macOS `TempDir` path
    // (`/var/folders/...`) never equals the `/private/var/folders/...` form the
    // CLI prints back.
...
impl IsolatedDataset {
    ...
    pub fn empty() -> std::io::Result<Self> {
        ...
        Ok(Self {
            ...
            beads_dir,
            metadata,
            source_dataset: KnownDataset::ThisRepo, // Placeholder
        })
    }
...
fn copy_workspace_failure_fixture_root(src: &Path, dst: &Path) -> std::io::Result<()> {
    ...
    // Fixtures store the workspace as a visible `beads/` and it is copied to
    // `.beads/` here. The visible name is not cosmetic: `br init` writes a
    // `.beads/.gitignore` that ignores the database and its sidecars, so a
    // fixture committed under its real name loses most of its payload to that
    // rule the moment anyone runs `git add` without `-f`.
    let payload = src.join("beads");
    if payload.exists() {
    ...
    for entry in fs::read_dir(src)? {
        ...
        let file_name = entry.file_name();
        let name = file_name.to_string_lossy();
        if matches!(name.as_ref(), "fixture.json" | "beads") {
            continue;
        }
...
pub fn isolated_from_override(
    ...
    Ok(IsolatedDataset {
        ...
        beads_dir,
        metadata,
        source_dataset: KnownDataset::ThisRepo, // Placeholder for overrides
    })
}
...
///
/// ```ignore
/// let mut guard = DatasetIntegrityGuard::new(KnownDataset::ThisRepo)?;
...
///
/// ```ignore
/// let provenance = run_with_integrity(KnownDataset::ThisRepo, |isolated| {
...
mod tests {
    ...
    fn isolated_beads_or_skip(test_name: &str) -> Option<IsolatedDataset> {
        let registry = DatasetRegistry::new();
        if !registry.is_available(KnownDataset::ThisRepo) {
            eprintln!("Skipping {test_name}: beads dataset not available (no beads.db in CI)");
            return None;
        }

        match IsolatedDataset::from_dataset(KnownDataset::ThisRepo) {
    ...
    fn test_registry_creation() {
        ...
        let _ = registry.is_available(KnownDataset::ThisRepo);
    }

    #[test]
    fn test_isolated_dataset_copy() {
        let Some(isolated) = isolated_beads_or_skip("test_isolated_dataset_copy") else {
            return;
        };
    ...
    fn beads_available() -> bool {
        DatasetRegistry::new().is_available(KnownDataset::ThisRepo)
    }
    ...
    fn test_source_integrity_check() {
        ...
        let result = registry.verify_source_integrity(KnownDataset::ThisRepo);
    ...
    fn test_integrity_guard_creation() {
        ...
        let guard =
            DatasetIntegrityGuard::new(KnownDataset::ThisRepo).expect("should create guard");
    ...
    fn test_isolated_from_override() {
        ...
        let override_cfg = DatasetOverride::new(
            KnownDataset::ThisRepo.source_path(),
    ...
    fn test_run_with_integrity() {
        ...
        let (result, provenance) = run_with_integrity(KnownDataset::ThisRepo, |isolated| {
    ...
    fn test_metadata_includes_source_commit() {
        let Some(isolated) = isolated_beads_or_skip("test_metadata_includes_source_commit") else {
            return;
        };

        let expected_commit = get_git_commit(&KnownDataset::ThisRepo.source_path());
    ...
    fn test_metadata_to_json_includes_new_fields() {
        let Some(isolated) = isolated_beads_or_skip("test_metadata_to_json_includes_new_fields")
        else {
            return;
        };
