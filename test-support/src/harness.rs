...
impl RunnerPolicy {
    ...
    pub fn ci() -> Self {
        Self {
            ...
            scenario_timeout: Duration::from_mins(1),
    ...
    pub fn local() -> Self {
        Self {
            ...
            scenario_timeout: Duration::from_mins(10),
            parallelism_mode: ParallelismMode::Serial,
    ...
    pub fn benchmark() -> Self {
        Self {
            command_timeout: Duration::from_hours(1),
            scenario_timeout: Duration::from_hours(2),
...
impl ArtifactLogger {
    pub fn new(suite: &str, test: &str) -> Self {
        let artifact_dir = crate::repo_root()
...
mod tests {
    ...
    fn test_runner_policy_builder() {
        let policy = RunnerPolicy::default()
            ...
            .with_scenario_timeout(Duration::from_mins(2))
    ...
    fn test_run_br_env_uses_override() {
        ...
        let value: serde_json::Value = serde_json::from_str(&payload).expect("parse info json");
        let path = value
            .get("beads_dir")
            .and_then(|p| p.as_str())
            .unwrap_or("");
