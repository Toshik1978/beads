...
pub mod test_helpers {
    ...
    /// Resolve an *expected* path the way the product resolves the path it
    /// returns, so an assertion compares like with like.
    ///
    /// The path-producing APIs in this crate canonicalize deliberately:
    /// `config::routing::follow_redirects` (which every `discover_beads_dir*`
    /// entry point funnels through) needs resolved paths for redirect-loop
    /// detection, and `cli::commands::stats::git_repo_context_from_filesystem`
    /// canonicalizes its start directory before walking up. On macOS a
    /// `TempDir` lives under `/var/folders/...`, which is a symlink to
    /// `/private/var/folders/...`, so a raw `TempDir` path never equals what
    /// those APIs hand back. Canonicalize the expectation here rather than
    /// relaxing the product to return unresolved paths.
    ///
    /// (These assertions used to pass only because `Taskfile.yml` ran the suite
    /// with `TMPDIR=/private/tmp/beads-test`, a path with no symlinked
    /// ancestor. That workaround is gone, which is what exposed them.)
    ///
    /// # Panics
    ///
    /// Panics if `path` cannot be canonicalized, which in a test means the
    /// expectation names a directory the test never created.
