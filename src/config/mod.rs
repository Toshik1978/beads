...
use crate::storage::conn::DbError;
use crate::storage::conn::SqliteValue;
...
/// header size) cannot be valid and will cause the previous engine to return
...
/// True when one of the database's sidecar paths is occupied by something
/// that is not a regular file — a directory, most often, left behind by a
/// half-finished copy or an archive extraction.
///
/// SQLite cannot produce this state itself and cannot recover from it: the
/// unix VFS touches `-journal` (hot-journal detection) and `-wal` (WAL open)
/// *before* it has read enough of the main file to judge it, so a blocked
/// sidecar aborts every open of the family no matter what the database
/// contains. Moving the blocker aside is exactly what JSONL recovery does,
/// which is why this is a recoverable condition rather than a fatal one.
///
/// # What counts as "not a regular file"
///
/// This uses `symlink_metadata`, so the test is on the sidecar path's own
/// entry rather than on whatever it resolves to. A directory is the case this
/// exists for, but a **symlink** counts too, even one pointing at a perfectly
/// good file, and so would a fifo or a socket. That is deliberate rather than
/// incidental, and it is safe in both directions:
///
/// - it never fires on a healthy workspace, because the only caller reaches it
///   after an open has already failed with a non-transient error;
/// - the recovery it enables renames the sidecar into `.beads/recovery/`, which
///   moves the *link* and leaves its target untouched.
///
/// The alternative — following the link and judging the target — would make
/// `br`'s behaviour depend on something outside the workspace, which is worse.
/// `reject_symlinked_database_path_for_recovery` takes the same view of a
/// symlinked *database* path, for the same reason.
fn database_family_has_blocking_sidecar(db_path: &Path) -> bool {
...
fn should_attempt_jsonl_recovery(open_err: &BeadsError, db_path: &Path, jsonl_path: &Path) -> bool {
    ...
    // Classify by the state of the database family, not only by the error
    // variant. Which variant a blocked sidecar produces is an engine detail:
    // measured against SQLite 3.50.6 with `beads.db` holding plain text, a
    // directory at `beads.db-wal` gives SQLITE_CANTOPEN and one at
    // `beads.db-journal` gives SQLITE_IOERR, where the same file with no
    // sidecar at all gives SQLITE_NOTADB. Only the last was in the set below,
    // so the two cases the product has explicit recovery code for — backing
    // the blocker up into `.beads/recovery/` and rebuilding from JSONL — never
    // reached it. Transient errors are excluded: a contended open must not
    // cause the database to be moved aside.
    if let BeadsError::Database(db_err) = open_err
        && !db_err.is_transient()
        && database_family_has_blocking_sidecar(db_path)
    {
        return true;
    }

    // `TableExists`, `IndexExists` and `NoSuchColumn` are the schema-shape
    // family: the file is readable, but its shape is one the migrations could
    // not bring forward, and rebuilding from JSONL is the route out.
    //
    // bds-04l.18 added `NoSuchColumn`. `run_pre_schema_migrations` now
    // backfills every table SCHEMA_SQL indexes, but two gaps are structural
    // rather than oversights: SQLite cannot ALTER TABLE ADD a PRIMARY KEY
    // column, nor a NOT NULL column with no DEFAULT. A legacy `labels` missing
    // `label`, say, is beyond repair by migration, and before this it failed
    // the open with a raw "no such column" and no way forward at all.
    //
    // `NotNullViolation` is deliberately NOT in this set. bds-04l.15 fixed its
    // known cause at source, and unlike the shape errors it can also mean a
    // row is bad rather than the schema — moving the database aside is too
    // heavy a response to that.
    matches!(
        open_err,
        BeadsError::Database(
...
fn rebuild_database_family(
    ...
    // Without this, the previous engine's post-import MVCC state can lag behind and
    ...
    // the previous engine's B-tree layer may leave behind during bulk import.
    ...
    // the previous engine's B-tree layer can introduce during bulk insert.
    ...
    // `PRAGMA integrity_check` report `ok` on a file produced by the previous engine's
    // bulk-insert + REINDEX path (issue #248). In-place VACUUM alone —
    // even called twice after a fresh checkpoint — does not truncate the
    // trailing pages that the previous engine's REINDEX leaves orphaned: those pages
    // exist in the file but are neither on the freelist nor referenced
    // from any B-tree root. `VACUUM INTO` sidesteps the previous engine's in-place
...
fn compact_database_via_vacuum_into_in_place_with_reopener(
    ...
    // state instead of having to reach into a WAL that the previous engine's own
...
impl OpenStorageResult {
    ...
    pub(crate) fn recover_database_from_jsonl(&mut self) -> Result<()> {
        ...
        // The previous engine tracked pages by file path, so keeping the old connection
...
///
/// This is the storage-policy switch for operators who want `issues.jsonl` to
/// be the single durable state file: flip this and stop the `.br_history/`
...
mod tests {
    ...
    use crate::util::test_helpers::canonical;
    use chrono::Utc;
    use tempfile::TempDir;
...
";
    ...
    fn discover_beads_dir_finds_at_root() {
        ...
        assert_eq!(discovered, canonical(&beads_dir));
    }
    ...
    fn discover_optional_beads_dir_with_cli_uses_explicit_db_override() {
        ...
        assert_eq!(discovered, Some(canonical(&beads_dir)));
    }
    ...
    fn discover_optional_beads_dir_with_cli_follows_redirect_for_explicit_db_override() {
        ...
        assert_eq!(discovered, Some(canonical(&target_beads)));
    }
...
";
    ...
    /// bds-04l.18. A legacy shape whose indexed column cannot be added by
    /// `ALTER TABLE` -- a PRIMARY KEY column, or NOT NULL with no DEFAULT --
    /// is beyond repair by migration. Before this it failed the open with a raw
    /// "no such column" and no route forward; it now rebuilds from JSONL, like
    /// the other schema-shape errors beside it.
    ///
    /// Its own test rather than another arm of
    /// `should_attempt_jsonl_recovery_only_for_corruption_errors`, which is
    /// already at clippy's 100-line ceiling.
    #[test]
    fn should_attempt_jsonl_recovery_for_an_unrepairable_missing_column() {
        let temp = TempDir::new().expect("tempdir");
        let beads_dir = temp.path().join(".beads");
        let db_path = beads_dir.join("beads.db");
        let jsonl_path = beads_dir.join("issues.jsonl");
        ...
        assert!(should_attempt_jsonl_recovery(
            &BeadsError::Database(DbError::NoSuchColumn {
        ...
        // Still gated on both files existing: with no JSONL there is nothing
        // to rebuild from, so the database must not be moved aside.
        fs::remove_file(&jsonl_path).expect("remove jsonl");
        assert!(!should_attempt_jsonl_recovery(
            &BeadsError::Database(DbError::NoSuchColumn {
                name: "label".to_string()
            }),
    ...
    /// A sidecar path occupied by a directory makes any open error recoverable.
    ///
    /// Which error a blocked sidecar produces is an engine detail, and it
    /// changed under `rusqlite`: measured against SQLite 3.50.6 with the
    /// database holding plain text, a directory at `beads.db-wal` gives
    /// SQLITE_CANTOPEN and one at `beads.db-journal` gives SQLITE_IOERR, while
    /// the same file with no sidecar gives SQLITE_NOTADB. Classifying on the
    /// variant alone therefore sent the two cases the product has explicit
    /// backup-and-rebuild code for straight to a fatal error. This pins the
    /// filesystem-shaped rule that replaced it, including the two limits that
    /// keep it from being a blanket "recover on anything".
    #[test]
    fn a_non_file_sidecar_makes_an_open_error_recoverable() {
        let temp = TempDir::new().expect("tempdir");
        let beads_dir = temp.path().join(".beads");
        let db_path = beads_dir.join("beads.db");
        let jsonl_path = beads_dir.join("issues.jsonl");
        fs::create_dir_all(&beads_dir).expect("create beads dir");
        fs::write(&db_path, b"not a sqlite database").expect("write db");
        fs::write(&jsonl_path, "{}\n").expect("write jsonl");

        let cannot_open = BeadsError::Database(DbError::CannotOpen {
            path: db_path.clone(),
        });
        let disk_io = BeadsError::Database(DbError::Internal("disk I/O error".to_string()));

        // Baseline: with a clean family neither error routes to recovery.
        assert!(!should_attempt_jsonl_recovery(
            &cannot_open,
            &db_path,
            &jsonl_path
        ...
        for (suffix, err) in [("-wal", &cannot_open), ("-journal", &disk_io)] {
            let blocker = beads_dir.join(format!("beads.db{suffix}"));
            fs::create_dir(&blocker).expect("create blocking sidecar");
            assert!(
                database_family_has_blocking_sidecar(&db_path),
                "a directory at beads.db{suffix} must count as a blocking sidecar"
            );
            assert!(
                should_attempt_jsonl_recovery(err, &db_path, &jsonl_path),
                "a directory at beads.db{suffix} must make {err} recoverable"
            );
            // Limit 1: a contended open must never move the database aside,
            // however odd the family looks.
            assert!(
                ...
                "a transient error must not trigger recovery"
            );
            assert!(!should_attempt_jsonl_recovery(
            ...
            fs::remove_dir(&blocker).expect("remove blocking sidecar");
        }

        // Limit 2: ordinary sidecar *files* are the normal state of a WAL
        // database and must not be mistaken for blockers.
        fs::write(beads_dir.join("beads.db-wal"), b"").expect("write wal file");
        fs::write(beads_dir.join("beads.db-shm"), b"").expect("write shm file");
        assert!(
            !database_family_has_blocking_sidecar(&db_path),
            "regular -wal/-shm files are not blockers"
        );
        assert!(!should_attempt_jsonl_recovery(
            &cannot_open,
            &db_path,
        ...
        // And the documented breadth: the check is on the sidecar path's own
        // entry (`symlink_metadata`), so a symlink counts as a blocker even
        // when its target is a perfectly good file. Recovery renames the link
        // and leaves the target alone, so this is safe as well as deliberate;
        // pinned here because the doc comment claims it.
        let target = beads_dir.join("elsewhere-wal");
        fs::write(&target, b"a real file").expect("write link target");
        fs::remove_file(beads_dir.join("beads.db-wal")).expect("clear wal file");
        std::os::unix::fs::symlink(&target, beads_dir.join("beads.db-wal")).expect("symlink wal");
        assert!(
            database_family_has_blocking_sidecar(&db_path),
            "a symlinked sidecar is judged by its own entry, not by its target"
        );
        assert!(
            target.is_file(),
            "the symlink's target must be untouched by the check itself"
        );
    }
