...
//! paginated JSON output. Supports text, JSON, and CSV formats.
...
fn execute_inner(
    ...
    if truncated && !quiet && !matches!(output_format, OutputFormat::Json) {
        if client_filters {
...
mod tests {
    ...
    ///
    /// Test-only wrapper: the production callers of this composition were the
    /// saved-query commands removed in bds-04l.2.6, but the validators it
