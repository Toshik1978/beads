...
/// Read-only git visibility for the JSONL export (beads#338).
///
...
/// `execute_import` is fragile on the previous engine — it trips stale-pager/MVCC
...
fn validate_sync_paths(
    ...
    // `jsonl_path` is deliberately left uncanonicalized above (see the
    // comment on `resolve_requested_sync_path`) so the earlier git/symlink
    // checks inspect what the operator actually asked for. That raw form is
    // wrong for *this* comparison, though: `beads_dir` here can arrive
    // already-canonical too (e.g. `resolve_sync_startup_paths` discovers it
    // via `config::discover_beads_dir_with_cli`, which always canonicalizes,
    // while a `--db` override can independently hand back a raw
    // `jsonl_path`), so neither "compare raw" nor "compare canonical" alone
    // is reliably consistent. Resolve `jsonl_path` to its best-effort real
    // location -- canonicalizing through the deepest existing ancestor, for
    // paths that don't exist yet -- and compare that against
    // `canonical_beads`. Comparing only the as-given forms is what let an
    // ordinary in-.beads path get misclassified as external whenever an
    // ancestor of `beads_dir` was itself a symlink, e.g. macOS $TMPDIR is
    // /var/folders/..., and /var is itself a symlink to /private/var.
    let is_external = !resolve_best_effort_canonical(&jsonl_path).starts_with(&canonical_beads);
    ...
    // Pass the *raw* `beads_dir`, not `canonical_beads`: `jsonl_path` is
    // deliberately left uncanonicalized above, and
    // `validate_sync_path_with_external`'s own internal containment check
    // compares against both the raw and canonicalized forms of whatever
    // `beads_dir` it's given. Passing the already-canonical form here would
    // collapse that fallback to a single (canonical-only) comparison and
    // misclassify an ordinary in-.beads path as external whenever an
    // ancestor of `beads_dir` is itself a symlink.
...
fn validate_operator_requested_sync_path(beads_dir: &Path, jsonl_path: &Path) -> Result<()> {
    ...
    // Delegate the actual ancestor walk to the same corrected implementation
    // `validate_sync_path` uses (`src/sync/path.rs`). This function used to
    // carry its own hand-rolled copy that accumulated `candidate` from the
    // filesystem root, which meant it walked ancestors *above* the beads
    // directory too -- on macOS $TMPDIR is /var/folders/..., and /var is
    // itself a symlink to /private/var, so that copy rejected every macOS
    // temp-dir fixture just like the original bug in path.rs did. Sharing
    // one implementation means the two call sites can no longer drift.
    // Match every returned variant, not just `SymlinkEscape`. Anything the
    // helper returns is a rejection, so an `if let` naming one variant would
    // silently *allow* the path on any other -- which is how the read_link
    // fail-open this guard now rejects (`SymlinkTargetUnreadable`) would have
    // slipped through here even after the helper was fixed.
    match symlink_escape_for_existing_ancestor(&operator_path, &canonical_beads) {
        Some(PathValidation::SymlinkEscape { path, target }) => {
            return Err(BeadsError::Config(format!(
...
/// Best-effort canonicalization for containment comparisons: canonicalizes
/// `path` directly if it exists, otherwise walks up to the deepest existing
/// ancestor, canonicalizes that, and re-appends the non-existent remainder
/// literally. Unlike `dunce::canonicalize`, this never fails on a path whose
/// leaf (or several trailing components) doesn't exist yet -- which is the
/// normal case for a JSONL export that hasn't been written, or an internal
/// parent directory sync is about to create.
fn resolve_best_effort_canonical(path: &Path) -> PathBuf {
    if let Ok(canonical) = dunce::canonicalize(path) {
        return canonical;
    }

    let mut missing_reversed: Vec<&std::ffi::OsStr> = Vec::new();
    let mut existing = path;
    loop {
        let Some(parent) = existing.parent() else {
            return path.to_path_buf();
        };
        missing_reversed.push(existing.file_name().unwrap_or_default());
        if let Ok(canonical_parent) = dunce::canonicalize(parent) {
            let mut resolved = canonical_parent;
            for component in missing_reversed.iter().rev() {
...
/// (beads#338). Never returns an error: any git failure collapses
...
fn execute_import(
    ...
    // trigger the previous engine's stale-pager OpenRead bug ("could not open storage
    ...
    // the previous engine's btree cursor bugs on DELETE operations in large tables.
    ...
    // generates "never used" freelist pages that the previous engine's VACUUM cannot
    ...
    let force_import_target_was_empty = if args.force || args.rebuild {
        ...
        if existing_issue_count == 0 && preserved_tombstones.is_empty() {
            debug!(
                "Force/rebuild import: target DB already empty, skipping reset_data_tables to avoid the previous engine's freelist leak"
            );
            true
        } else {
            debug!(
    ...
    if (args.force || args.rebuild || import_rewrote_storage) && !skip_heavy_import_maintenance {
        ...
        // on matches what's actually on disk. Without this, the previous engine's
        ...
        // Final compaction via `VACUUM INTO` + atomic rename.  The previous engine's
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn test_validate_operator_requested_sync_path_accepts_symlinked_ancestor() {
        use std::os::unix::fs::symlink;

        // macOS $TMPDIR is /var/folders/…, and /var is a symlink to
        // /private/var. A .beads directory legitimately sitting under a
        // symlinked ancestor must not be treated as an escape.
        // validate_operator_requested_sync_path used to carry its own
        // hand-rolled ancestor walk that accumulated from the filesystem
        // root -- the same directional bug fixed in
        // symlink_escape_for_existing_ancestor (src/sync/path.rs) -- and
        // would have rejected this as an escape at /var. It now delegates to
        // that shared, corrected implementation.
        ...
        let result = validate_operator_requested_sync_path(&beads_dir, &jsonl_path);
        assert!(
            result.is_ok(),
            "a .beads dir under a symlinked ancestor must be accepted, got: {result:?}"
        );
    }
