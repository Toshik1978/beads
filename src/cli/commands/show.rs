...
fn format_issue_details(details: &IssueDetails, use_color: bool, wrap: bool) -> String {
    ...
    let _ = writeln!(
        output,
        "Created: {} · Updated: {}",
        to_local(issue.created_at).format("%Y-%m-%d"),
    ...
    if !details.comments.is_empty() {
        ...
        for comment in &details.comments {
            let _ = writeln!(
                output,
                "  [{}] {}: {}",
                to_local(comment.created_at).format("%Y-%m-%d %H:%M %:z"),
...
mod tests {
    ...
    /// bds-04l.9. `show_fields` is the authority for which fields `show`
    /// renders, and the Rich panel is fed by it directly. The Plain renderer
    /// keeps its own bespoke layout -- it packs Owner onto the Type line and
    /// Created onto the Updated line, which a shared row-formatter cannot
    /// produce without a layout switch nobody needs -- so it is held to the
    /// same list here instead. Adding a field to `show_fields` fails this test
    /// until Plain emits it too, which is what stops the two renderers drifting
    /// apart again.
    #[test]
    fn plain_renderer_covers_every_shared_field() {
        let issue = Issue {
            id: "bd-fields".to_string(),
            title: "Field coverage".to_string(),
            ...
            acceptance_criteria: Some("AC-MARKER".to_string()),
            notes: Some("NOTES-MARKER".to_string()),
            ..Issue::default()
        };

        let rows = show_fields::metadata_rows(&issue);
        // Owned, so the fixture can be moved into IssueDetails below while the
        // expected bodies stay available for the assertions.
        let sections: Vec<(&'static str, String)> = show_fields::prose_sections(&issue)
            .into_iter()
            .map(|section| (section.heading, section.body.to_string()))
            .collect();
        assert_eq!(
            rows.len(),
            6,
        ...
        for row in &rows {
            assert!(
                rendered.contains(&row.value),
                "the Plain renderer omits the value of `{}` ({}), which `show_fields` declares. \
                 Full output:\n{rendered}",
                row.label,
                row.value
            );
        ...
        for (heading, body) in &sections {
            assert!(
                ...
                "the Plain renderer omits the `{heading}` heading. Full output:\n{rendered}"
            );
            assert!(
