...
/// The `.gitignore` every `br init` writes into a new `.beads/` workspace.
///
/// Named rather than inlined at the write site so that it can be compared
/// against a real workspace's copy. `init` writes this file only when it is
/// missing — which is correct, since a user may have edited theirs — but it
/// also means an existing workspace never picks up changes made here. This
/// repository's own `.beads/.gitignore` silted up exactly that way, keeping
/// daemon-runtime entries for a daemon that no longer exists long after this
/// template dropped them. `gitignore_template_matches_this_workspace` is the
/// guard against a repeat.
pub const WORKSPACE_GITIGNORE: &str = r"# Every pattern here is anchored with a leading `/`, so it matches only at the
# top level of this directory. Without that, `*.db` and `*.lock` also match
# inside `attachments/` — which is meant to be committed — and would silently
# drop an attachment named `notes.db`.
...
# — same lifecycle as .br_history/, written by the recovery paths.
# Filename suffix `.truncated-wal` slips past the
...
mod tests {
    ...
    fn test_gitignore_excludes_db_files() {
        ...
        assert!(
            ...
            "init must emit the canonical write-lock rule, anchored: {content}"
        );
        assert!(content.contains("*.lock"));
        info!("test_gitignore_excludes_db_files: assertions passed");
    }

    /// This repository's own `.beads/.gitignore` must equal what `init` writes.
    ///
    /// NOT a redundant restatement of `test_gitignore_excludes_db_files`. That
    /// one runs `init` into a temp dir and checks the file it just produced, so
    /// it can only ever agree with the template — it is blind to a workspace
    /// created by an older build. This one compares the template to a workspace
    /// on disk that was created years of commits ago, which is the only place
    /// the drift can show up.
    ///
    /// The drift is not hypothetical and the mechanism is not a bug: `init`
    /// writes `.gitignore` only when it is missing, deliberately, so that a
    /// user's edits survive. The cost is that an existing workspace never
    /// receives later changes. This repository's copy kept `daemon.lock`,
    /// `daemon.log`, `daemon.pid`, `bd.sock` and `sync-state.json` long after
    /// the daemon was removed and this template stopped emitting them.
    ///
    /// If this fails after a deliberate template change, the fix is to copy the
    /// new template over `.beads/.gitignore` — not to weaken the assertion.
    #[test]
    fn gitignore_template_matches_this_workspace() {
        ...
        let on_disk = fs::read_to_string(&path).unwrap_or_else(|error| {
            panic!(
                "this repository's own {} must be readable: {error}",
                path.display()
            )
        });
        ...
        assert_eq!(
            on_disk,
            WORKSPACE_GITIGNORE,
            "{} has drifted from the template `br init` writes.\n\
             Copy the template over it rather than relaxing this test.",
            path.display()
        );
    }
}
