...
/// inconsistency (e.g. the previous engine's prepared-statement / pager cache edge case,
...
pub fn execute(args: &UpdateArgs, cli: &config::CliOverrides, ctx: &OutputContext) -> Result<()> {
    ...
    // close-policy / delete pipelines are applied.
...
fn prepare_single_route(
    ...
    validate_mutable_target_issues(
        &storage_ctx.storage,
        &resolved_ids,
        has_updates,
...
/// Rejects updates whose *target* issue makes the update illegitimate,
/// as opposed to [`reject_terminal_status_transition`], which judges the
/// requested status alone and so runs before any I/O.
///
/// Two rules, and both need the issue's current status from storage:
///
/// * a `tombstone` issue is not updatable at all;
/// * a terminal issue may not be moved *out* of its terminal state, for the
///   same reason `reject_terminal_status_transition` refuses to move one *in*.
fn validate_mutable_target_issues(
    storage: &SqliteStorage,
    ids: &[String],
    has_updates: bool,
    requested_status: Option<&Status>,
) -> Result<()> {
    if !has_updates {
        return Ok(());
    }
    ...
    for id in ids {
        ...
        // bds-04l.7. `reject_terminal_status_transition` covers only half of
        // its own argument: it stops `update` from *entering* a terminal state
        // so that `close`/`delete` keep sole ownership of that audit contract,
        // but nothing stopped it from *leaving* one. `br update <id> --status
        // deferred` on a closed issue exited 0 and cleared `closed_at` and
        // `close_reason` along with the status, so the issue was silently
        // un-closed with `br reopen` never invoked and the close-policy audit
        // trail recording nothing. The removed `defer` command was the only
        // caller enforcing this direction (it skipped any
        // `issue.status.is_terminal()`), which is why removing it in
        // bds-04l.2.6 exposed the asymmetry rather than causing it.
        //
        // Refusing -- rather than quietly routing through `reopen` -- is what
        // keeps one transition to one command: an `update` that performed a
        // reopen would recreate the very "two different audit contracts"
        // problem the sibling guard exists to prevent.
            ...
            && !new_status.is_terminal()
        {
            return Err(BeadsError::validation(
                "status",
                // Deliberately free of hyphenated words: the snapshot suite's
                // ID redaction matches `\b[\w-]+-[a-z0-9]{3,}\b`, so a
                // phrasing like "un-close" is rewritten to ID-REDACTED in
                // every golden that captures this error and the message
                // becomes unreadable there.
                format!(
                    "refusing to move {id} out of a terminal state via \
                     `br update --status {}`: leaving a terminal state must go \
                     through `br reopen` so the close audit trail (closed_at / \
                     close_reason) is retired deliberately rather than silently \
                     cleared. Use `br reopen {id}` instead.",
                    new_status.as_str()
                ),
            ));
        }
...
mod tests {
    ...
    fn test_build_update_with_status() {
        ...
        // `reject_terminal_status_transition` — see the dedicated tests
        // below.
    ...
    /// Tombstone is also a terminal state with a dedicated
    ...
    fn test_validate_mutable_target_issues_rejects_tombstone() {
        ...
        let err =
            validate_mutable_target_issues(&storage, &["bd-tombstone".to_string()], true, None)
                .unwrap_err();
    ...
    fn test_validate_mutable_target_issues_allows_open_issue() {
        ...
        validate_mutable_target_issues(&storage, &["bd-open".to_string()], true, None).unwrap();

        info!("test_validate_mutable_target_issues_allows_open_issue: assertions passed");
    }

    /// bds-04l.7. Builds a storage holding one closed issue and returns it.
    fn storage_with_closed_issue() -> SqliteStorage {
        let mut storage = SqliteStorage::open_memory().unwrap();
        let issue = Issue {
    ...
    #[test]
    fn validate_mutable_target_issues_refuses_to_un_close_a_closed_issue() {
        init_test_logging();

        let storage = storage_with_closed_issue();

        let err = validate_mutable_target_issues(
            &storage,
            &["bd-closed".to_string()],
            true,
            Some(&Status::Deferred),
        )
        .unwrap_err();
        ...
        assert!(
            reason.contains("br reopen"),
            "the refusal must name the command that owns this transition, got: {reason}"
        );
    }

    /// The guard must refuse only the status change, not every update to a
    /// closed issue: adding notes or labels to closed work stays legitimate.
    #[test]
    fn validate_mutable_target_issues_allows_non_status_updates_to_a_closed_issue() {
        init_test_logging();
