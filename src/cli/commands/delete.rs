    ...
    fn test_cascade_dependents_collection() {
        ...
        storage
            .mutate("test_add_deps", |tx, _ctx| {
                ...
                tx.execute_with_params(
                    "INSERT INTO dependencies (issue_id, depends_on_id, type, created_at) VALUES (?, ?, ?, ?)",
                    &[SqliteValue::from("bd-c"), SqliteValue::from("bd-b"), SqliteValue::from("blocks"), SqliteValue::from(chrono::Utc::now().to_rfc3339().as_str())],
                )?;
                Ok(())
            })
            .unwrap();
    ...
    fn test_preview_cascade_closure_exceeds_direct_dependents() {
        ...
        storage
            .mutate("test_add_chain_deps", |tx, _ctx| {
                ...
                tx.execute_with_params(
                    "INSERT INTO dependencies (issue_id, depends_on_id, type, created_at) VALUES (?, ?, ?, ?)",
                    &[SqliteValue::from("bd-d"), SqliteValue::from("bd-c"), SqliteValue::from("blocks"), SqliteValue::from(chrono::Utc::now().to_rfc3339().as_str())],
                )?;
                Ok(())
            })
            .unwrap();
