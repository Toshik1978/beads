...
struct ProjectionInfo {
    ...
    /// `"clear"`, `"diverges"`, or `"unavailable"` — mirrors how
    /// `parity_status`/`ready_parity_status` name an explicit unavailable
    /// state rather than letting a failed query masquerade as a healthy
    /// zero. Always present, unlike the `Option` fields below: a consumer
    /// must be able to tell "checked, found none" from "could not check"
    /// without cross-referencing anything else.
    hierarchy_divergence_status: String,
    #[serde(skip_serializing_if = "Option::is_none")]
...
fn build_projection_info(conn: &Connection) -> ProjectionInfo {
    ...
    // A failed query must not read as a healthy zero: `hierarchy_divergence_status`
    // carries "unavailable" separately from "clear" so the two are never
    // conflated, the same distinction `parity_status`/`ready_parity_status`
    // already draw for the neighbouring cache-parity checks.
    let (hierarchy_divergence_status, hierarchy_divergence_rows, hierarchy_divergence_ids) =
        match SqliteStorage::hierarchy_divergences_for(conn) {
            Ok(divergences) => {
                ...
                let ids = divergences
                    ...
                    .map(|(id, _, _)| id.clone())
                    .collect();
                (status.to_string(), Some(divergences.len()), ids)
            }
            Err(_) => ("unavailable".to_string(), None, Vec::new()),
        };
    ...
    ProjectionInfo {
        ...
        child_counter_rows,
        hierarchy_divergence_status,
        hierarchy_divergence_rows,
        hierarchy_divergence_ids,
    }
}
...
fn print_projection_human(projections: &ProjectionInfo) {
    ...
    if projections.hierarchy_divergence_status == "unavailable" {
        // Distinct from the "print nothing when clear" branch below: a
        // failed check is not a healthy workspace, and staying silent here
        // would make the two indistinguishable.
        println!("  Hierarchy divergence: unavailable (query failed)");
    } else if let Some(count) = projections.hierarchy_divergence_rows
        && count > 0
    {
...
mod tests {
    ...
    fn test_collect_info_output_does_not_create_missing_db() {
        ...
        assert_eq!(
            ...
            canonical(&beads_dir).join("beads.db").display().to_string()
        );
    }
