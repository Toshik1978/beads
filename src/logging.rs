...
fn default_filter(verbosity: u8, quiet: bool) -> String {
    ...
    // These directives used to carry per-target caps for the pure-Rust storage
    // engine and its submodules (btree cells, VDBE steps, pager I/O), which
    // fired at `debug` for every row and page touched and drowned out beads's
    // own logs on bulk imports. `rusqlite` emits no `tracing` events at all, so
    // there is nothing left to cap and the directives are now beads-only.
    match verbosity {
        0 => {
...
mod tests {
    ...
    fn default_filter_varies_with_verbosity() {
        // `-v` and `-vv` no longer differ from the default in a debug build:
        // the levels they used to raise belonged to the storage engine's own
        // `tracing` targets, and `rusqlite` publishes none. Only the step up to
        // `trace` at `-vvv` still moves beads's own level.
        assert_eq!(default_filter(1, false), "beads=debug");
        assert_eq!(default_filter(0, false), "beads=debug");
        assert_eq!(default_filter(2, false), "beads=debug");
