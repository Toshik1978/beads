...
/// Per-statement busy timeout before SQLite returns SQLITE_BUSY.
///
/// MEASURED, not inherited. This was `0` for the same reason the MSRV was
/// wrong: the value came from a storage engine that is no longer here. Issue
/// #243 chose 0 because that engine implemented `busy_timeout` as a hot spin
/// loop which burned 100% CPU and starved the competing writer; C SQLite's
/// busy handler sleeps, so the argument died with the engine and the value had
/// never been re-derived against the one actually in use.
///
/// Re-derived under `bds-04l.17` with `examples/busy_timeout_contention.rs` —
/// N processes, each with its own `SqliteStorage` on one shared database file
/// and no flock between them, all calling `create_issue`. That is the real
/// population for this constant: `src/main.rs` serializes every mutating CLI
/// command behind a blocking flock on `.beads/.write.lock`, and
/// `config/mod.rs` resolves `.or(Some(30000))` before every startup open, so
/// the `br` binary neither reaches this value nor contends at the SQLite
/// layer. It reaches `SqliteStorage::open`, `build_memory`, and library
/// consumers, and only the last of those can contend.
///
/// Median of 3 reps, 150 writes per writer, on the host this was ported on
/// (`cargo run --release --example busy_timeout_contention`):
///
/// | busy_timeout | writers | writes/s | p50 | p99 | max | failures |
/// | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
/// | 0 ms | 4 | 1875 | 0.1 ms | 1.7 ms | 293 ms | 0 |
/// | 0 ms | 8 | 834 | 0.1 ms | 2.1 ms | 1408 ms | 0 |
/// | 0 ms | 16 | 410 | 0.1 ms | 2.8 ms | 5824 ms | 0 |
/// | 0 ms | 32 | 703 | 0.1 ms | 3.5 ms | 6802 ms | 12 |
/// | 5000 ms | 8 | 4989 | 0.1 ms | 1.4 ms | 220 ms | 0 |
/// | 5000 ms | 16 | 3815 | 0.1 ms | 2.6 ms | 602 ms | 0 |
/// | 5000 ms | 32 | 3485 | 0.1 ms | 5.1 ms | 1345 ms | 0 |
/// | 30000 ms | 16 | 3280 | 0.1 ms | 2.1 ms | 706 ms | 0 |
/// | 30000 ms | 32 | 3535 | 0.1 ms | 5.1 ms | 1329 ms | 0 |
///
/// Three findings, each of which contradicts the case for 0:
///
/// 1. **0 costs an order of magnitude of throughput.** It degrades from 1875
///    writes/s at 4 writers to 410 at 16 — 9x below the non-zero values, which
///    stay flat across the whole range. All of that time is inside
///    `create_issue`, not in process startup: a separate instrumented run
///    confirmed each writer's summed call latency equals its wall-clock write
///    phase to within 0.2 ms.
/// 2. **0 loses writes.** At 32 writers it produced hard failures — 12 in one
///    sweep, 22 in a second at different parameters — where the non-zero values
///    produced none at any writer count. Those are writes that exhausted
///    `with_write_transaction`'s 8 retries and returned an error to the caller.
///    The 703 writes/s in that row is higher than the 16-writer row only
///    because failed writes are not counted as work.
/// 3. **The jitter argument was backwards.** The case for 0 was that
///    `BEGIN IMMEDIATE` failing fast lets the *jittered* backoff desynchronize
///    writers, where an in-engine timeout has no jitter. Jitter is not the
///    binding constraint — granularity is. A write here costs ~0.1 ms and the
///    retry loop's backoff floor is 50 ms, so losing one race costs ~500x the
///    work it protects. SQLite's own busy handler retries at a granularity
///    matched to the lock, which is why the max-latency column runs 5824 ms at
///    0 against 602 ms at 5000 for the same 16 writers.
///
/// 5000 and 30000 are indistinguishable in every column, so the choice between
/// them is not a throughput question. 5000 wins on the pathological case: this
/// timeout is per `BEGIN IMMEDIATE` attempt and `with_write_transaction` retries
/// 8 times *on top of* it, so a writer that never gets in waits ~53 s at 5000
/// and ~4 minutes at 30000. Contention fully resolves within 1.4 s at 32
/// writers, so 5000 already carries roughly 3.5x headroom and the extra 25 s
/// buys nothing measurable. It also happens to match what `rusqlite` installs
/// at open on its own — a convenience if the explicit PRAGMA is ever dropped,
/// not the reason for the value.
///
/// Pinned by `test_default_busy_timeout_is_measured_not_inherited`, which fails
/// if this changes without a fresh sweep. Re-run the benchmark before moving
/// it: these are one host's numbers, and the ranking is what should be checked
/// to reproduce, not the absolute rates.
...
// The previous engine started returning false PRIMARY KEY conflicts when we rewrite
...
pub struct SqliteStorage {
    ...
    /// its WAL/SHM/journal sidecars — when the connection is dropped. The previous engine
...
impl SqliteStorage {
    ...
    fn refresh_blocked_cache_after_commit(
        ...
        // can only be changed outside an active transaction.  The previous engine can
    ...
    pub fn open_with_timeout(path: &Path, lock_timeout_ms: Option<u64>) -> Result<Self> {
        ...
        // Set busy_timeout unconditionally, so no engine's own default ever
        // decides it. `rusqlite` calls `sqlite3_busy_timeout(db, 5000)` at
        // open; the engine it replaced installed none, leaving SQLite's own
        // default of 0. Four `br sync` call sites pass `cli.lock_timeout`
        // straight through without the `.or(Some(30000))` the rest of the CLI
        // applies (`cli/commands/sync.rs:2525`, `:2533`, and
        // `config/mod.rs:1514`/`:1570` via `rebuild_database_family`), so a
        // `None` here would silently move them from fail-fast to a 5s block.
        // Applying the default here closes all four without touching them.
        //
        // On the value itself: `BEGIN IMMEDIATE` now waits inside SQLite for up
        // to `DEFAULT_BUSY_TIMEOUT_MS` before returning SQLITE_BUSY, and
        // `with_write_transaction`'s jittered backoff retries on top of that.
        // The in-engine wait is what carries the load — it retries at a
        // granularity matched to the lock, where the application backoff's
        // floor is 50 ms against a ~0.1 ms write. The `.write.lock` flock
        // serializes concurrent mutating *CLI* processes before they reach this
        // point; library consumers have no such flock, which is the population
        // the default was measured against. See `DEFAULT_BUSY_TIMEOUT_MS`.
        let timeout_ms = lock_timeout_ms.unwrap_or(DEFAULT_BUSY_TIMEOUT_MS);
    ...
    ///
    /// The previous engine required real file paths for WAL and schema operations, so
    ...
    ///
    /// Used before force imports to avoid the previous engine's btree cursor bugs on DELETE
        ...
        F: FnMut(&mut Self) -> Result<R>,
    {
        ...
        // or deadlocks. This loop is the *outer* wait: BEGIN IMMEDIATE first
        // blocks inside SQLite for up to DEFAULT_BUSY_TIMEOUT_MS, and only a
        // timeout there lands here. 8 retries × (50ms → 6400ms) gives ~12.7s
        // of additional jittered backoff on top of the in-engine wait.
        //
        // The two are not interchangeable, and the split matters. Measurement
        // under bds-04l.17 (see DEFAULT_BUSY_TIMEOUT_MS) showed this loop is a
        // poor substitute for the in-engine wait rather than an improvement on
        // it: its 50 ms floor is ~500x a typical write, so when it was doing
        // all the waiting (busy_timeout=0) throughput fell 9x at 16 writers and
        // writes began failing outright at 32. It earns its place as the
        // backstop for what busy_timeout cannot cover — a transient COMMIT, or
        // contention that outlives the in-engine timeout — not as the primary
        // desynchronizer.
        const MAX_RETRIES: u32 = 8;
        let base_backoff_ms: u64 = 50;
        let mut last_error: Option<crate::error::BeadsError> = None;
        ...
        let tx_result: Result<_> = self.with_write_transaction(|storage| {
            ...
            if !ctx.dirty_ids.is_empty() {
                ...
                for chunk in dirty_vec.chunks(DIRTY_ISSUE_CHUNK_SIZE) {
                    ...
                    // The previous engine did not reliably support UNIQUE constraint upserts.
    ...
    // 138 lines against clippy's 100-line pedantic limit. Accepted rather than
    // split.
    //
    // CORRECTION (bds-04l.4.2): this comment used to say the allow expires
    // "when bds-04l.4 rewrites the body". That rewrite never happened and is
    // not going to. bds-04l.4 replaced the storage engine through a compat
    // shim (`src/storage/conn.rs`) precisely so that this file would *not* be
    // rewritten — the port touched its `use` lines and nothing else. The
    // function is therefore exactly as long as it was, for its own reasons,
    // and this allow has no scheduled expiry. Splitting it is a standalone
    // refactor someone may still choose to do on its merits.
    //
    // Scoped to this function, not the crate, so a genuinely bloated function
    // written later still trips the lint.
    #[allow(clippy::too_many_lines)]
    fn enforce_workflow_transition_batch_in_tx(
        ...
        for (id, update) in updates {
            ...
            let violations = crate::close_policy::evaluate_transition_required_fields(
                workflow,
                id,
                Some(from),
                to,
    ...
    /// The unwindowed `br list` body, before any `LIMIT`/`OFFSET` suffix.
    ///
    /// Held as a constant for the same reason as
    /// [`Self::LIST_ACTIVE_PRIORITY_WINDOW_SQL`]: the query-plan guard
    /// (`unwindowed_list_query_stays_on_the_active_order_partial_index`) must
    ...
    pub fn list_text_issues_for_command_output(&self, filters: &ListFilters) -> Result<Vec<Issue>> {
        ...
        let mut sql = String::from(Self::LIST_ACTIVE_UNWINDOWED_SQL);
        match (filters.limit, filters.offset) {
            (Some(limit), offset) if limit > 0 => {
                let _ = write!(sql, " LIMIT {limit}");
                if let Some(offset) = offset
    ...
    /// One priority band of the `br list --limit N` window.
    ///
    /// Held as a constant so the query-plan guard
    ...
    fn list_text_issues_by_priority_window(
        ...
        for priority in Priority::CRITICAL.0..=Priority::BACKLOG.0 {
            ...
            let rows = self.conn.query_with_params(
                Self::LIST_ACTIVE_PRIORITY_WINDOW_SQL,
                &[
    ...
    /// structured fields used only by JSON/CSV or client-side filters.
    ///
    /// # Errors
    ///
    /// Returns an error if the database query fails.
    ...
    ///
    /// Hydrates only the columns consumed by the command's JSON/text
    ...
    /// Structured JSON callers should use
    ...
    fn build_ready_issue_candidates_query(
        ...
        // names (never raw user input reaching SQL), and the embedded previous-engine
    ...
    fn reset_blocked_cache_table(conn: &Connection) -> Result<()> {
        ...
        // previous-engine page leak (#224): DROP TABLE leaves old root pages
        ...
        // the previous engine could retain after bulk DELETE (#215).
    ...
    fn insert_blocked_cache_entries(
        ...
        // this function.  The previous engine could surface false FK violations when its page
    ...
    ///
    /// Hydrates only the issue columns consumed by blocked JSON/text
    /// output, avoiding large overflow-page reads for fields that never reach
    /// the user-facing result.
    ///
    /// # Errors
    ...
    pub fn get_epic_counts(&self) -> Result<std::collections::HashMap<String, (usize, usize)>> {
        ...
        // which crashed the previous engine (it doesn't support non-column arguments in aggregate functions).
        let rows = self.conn.query(
            "SELECT
    ...
    fn update_child_counter_in_tx(
        ...
        if i64::from(child_number) > current_max {
            // DELETE + INSERT to simulate UPSERT (a limitation of the previous engine).
            // FK enforcement is disabled by the caller's transaction wrapper
            // to avoid false FK violations from the previous engine (#215).
            conn.execute_with_params(
                "DELETE FROM child_counters WHERE parent_id = ?",
                &[SqliteValue::from(parent_id)],
            )?;
    ...
    pub fn count_relation_counts_for_issues(
        ...
        // collision trigger for the previous engine's MemDatabase.
        const SQLITE_VAR_LIMIT: usize = 900;
...
fn finish_issue_mutation_write_probe(
    ...
    match (probe_result, rollback_result) {
        ...
        (Ok(0), rollback) => {
            ...
            Err(BeadsError::Database(DbError::Internal(
...
fn query_external_project_capabilities(
    ...
    // Explicitly close the connection to avoid the previous engine's drop_close warnings.
    let _ = conn.close();
    Ok(satisfied)
}
...
impl SqliteStorage {
    ...
    fn insert_label_refs_for_import(
        ...
        // Keep label inserts single-row: the previous engine could mis-handle multi-values
...
mod tests {
    ...
    /// the previous engine could not use `:memory:`), and that file plus any WAL/SHM/
    ...
    fn test_event_insert_for_missing_issue_succeeds_with_fk_disabled() {
        ...
        // around the previous engine's false FK violations (#215).  Events for
    ...
    */
    ...
    /// `br list --limit N` must be *served by* `idx_issues_list_active_order`.
    ///
    ...
    ///    which no assertion about *results* can see.
    #[test]
    fn list_limit_window_stays_on_the_active_order_partial_index() {
        let storage = SqliteStorage::open_memory().unwrap();
        let plan = storage
            .conn
            .query_with_params(
                ...
                &[SqliteValue::from(1i64), SqliteValue::from(1i64)],
            )
            .expect("the windowed list query must have a query solution")
            .iter()
            .filter_map(|row| row.get(3).and_then(SqliteValue::as_text).map(String::from))
            .collect::<Vec<_>>()
            .join(" | ");

        assert!(
            plan.contains("idx_issues_list_active_order"),
            "the windowed list query must be served by the partial index, got: {plan}"
        );
        assert!(
            plan.contains("SEARCH"),
            "the partial index must be used as a lookup, not merely scanned: {plan}"
        );

        // Regression 2 needs a separate assertion, and it pins a *decision*
        ...
        assert!(
            ...
            "the mandatory index hint must stay: it is the deliberate guarantee \
             that this query reaches idx_issues_list_active_order rather than \
             relying on the planner choosing it"
        );
    }
    ...
    /// copy, which meant reverting the product query left the test green — the
    /// exact drift the constants exist to prevent.
    #[test]
    fn unwindowed_list_query_stays_on_the_active_order_partial_index() {
        let storage = SqliteStorage::open_memory().unwrap();
        let plan = storage
            .conn
            .query(&format!(
                "EXPLAIN QUERY PLAN {}",
                SqliteStorage::LIST_ACTIVE_UNWINDOWED_SQL
            ))
            .expect("the unwindowed list query must plan")
            .iter()
            .filter_map(|row| row.get(3).and_then(SqliteValue::as_text).map(String::from))
            .collect::<Vec<_>>()
            .join(" | ");
        ...
        assert!(
            plan.contains("idx_issues_list_active_order"),
            "the unwindowed list query must use the partial index, got: {plan}"
        );
    }
    ...
    /// The constant was unpinned for its whole life: `test_open_uses_default_busy_timeout`
    /// passes at 0, 5000 and 30000 alike, so nothing would have noticed a
    /// change. This pins both halves — the measured value, and the fact that
    /// `open` actually installs it on the connection rather than leaving
    /// `rusqlite`'s own default to decide.
    #[test]
    fn test_default_busy_timeout_is_measured_not_inherited() {
        /// A value that is nobody's default, so the read-back below proves the
        /// PRAGMA is actually applied rather than merely agreeing with an
        /// engine default that happens to match.
        const ARBITRARY_MS: u64 = 1_234;

        assert_eq!(
            DEFAULT_BUSY_TIMEOUT_MS, 5_000,
            "DEFAULT_BUSY_TIMEOUT_MS is a measured value, not a guess. Changing it means \
             re-running `cargo run --release --example busy_timeout_contention` and updating \
             the table in the constant's doc comment with the new numbers. The last sweep \
             found 0 costs ~9x throughput at 16 concurrent writers and loses writes outright \
             at 32, while 5000 and 30000 are indistinguishable."
        );

        let temp = TempDir::new().unwrap();
        let read_back = |storage: &SqliteStorage| {
            storage
                .conn
        ...
        let default_db = temp.path().join("busy_timeout_default.db");
        let default_storage = SqliteStorage::open(&default_db).unwrap();
        assert_eq!(
            read_back(&default_storage),
            Some(i64::try_from(DEFAULT_BUSY_TIMEOUT_MS).unwrap()),
            "open() should install DEFAULT_BUSY_TIMEOUT_MS on the connection"
        );

        // The assertion above cannot stand alone: `rusqlite` calls
        // `sqlite3_busy_timeout(db, 5000)` at open, so it would still pass if
        // `open_with_timeout`'s PRAGMA were deleted outright.
        let explicit_db = temp.path().join("busy_timeout_explicit.db");
        let explicit_storage =
            SqliteStorage::open_with_timeout(&explicit_db, Some(ARBITRARY_MS)).unwrap();
        assert_eq!(
            read_back(&explicit_storage),
            Some(i64::try_from(ARBITRARY_MS).unwrap()),
            "open_with_timeout should apply the caller's busy timeout, not an engine default"
        );
    }
    ...
    fn test_open_repairs_missing_canonical_indexes_even_when_user_version_is_current() {
        ...
        // Use PRAGMA index_list instead of sqlite_master (more reliable in the previous engine)
    ...
    fn test_open_repairs_legacy_kv_primary_key_tables() {
        ...
        // return inconsistent results in the previous engine).
    ...
    fn test_get_ready_issues_treats_null_legacy_flags_as_false() {
        ...
        // The step that makes the NULLs a non-problem, and the step every real
        // open performs.
    ...
    fn test_reset_data_tables_preserves_config() {
        // Use a real file to avoid the previous engine's in-memory BUSY contention under parallel tests.
        let temp = TempDir::new().unwrap();
    ...
    /// `PRAGMA wal_checkpoint(TRUNCATE)` is the storage engine's responsibility
    ...
    /// avoids a false alarm whenever the engine revises its WAL
    /// teardown.
    #[test]
