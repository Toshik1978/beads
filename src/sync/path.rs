...
pub enum PathValidation {
    ...
    /// Path is a confirmed symlink whose target could not be read, so it is
    /// unknown whether it escapes.
    ///
    /// Distinct from [`Self::SymlinkEscape`] because we have not shown the
    /// link escapes -- only that we cannot show it does not. Both are
    /// rejections: a path-escape check must fail closed.
    SymlinkTargetUnreadable { path: PathBuf, error: String },
...
impl PathValidation {
    ...
    pub fn rejection_reason(&self) -> Option<String> {
        match self {
            ...
            Self::SymlinkTargetUnreadable { path, error } => Some(format!(
                "Symlink '{}' could not be read ({error}), so it cannot be shown to stay inside the beads directory",
                path.display()
            )),
...
/// Walks `path`'s ancestors strictly below `canonical_beads`, looking for a
/// symlink whose resolved target escapes `canonical_beads`.
///
/// Shared by [`validate_sync_path`] and
/// `crate::cli::commands::sync::validate_operator_requested_sync_path`, which
/// both need to answer this exact question and previously each carried their
/// own hand-rolled ancestor walk. The two copies drifted: this is the
/// corrected version (see the comment below for why the walk must stop at
/// `canonical_beads` rather than continuing to the filesystem root). Keep
/// this the single implementation -- do not let a second copy grow back.
pub(crate) fn symlink_escape_for_existing_ancestor(
    path: &Path,
    canonical_beads: &Path,
) -> Option<PathValidation> {
    // Only ancestors strictly *below* canonical_beads can ever be a real escape.
    // An ancestor at or above the beads directory resolves to a path that
    // *contains* canonical_beads, never one contained by it, so `starts_with`
    // can only ever fail there -- e.g. on macOS $TMPDIR is /var/folders/...
    // and /var is itself a symlink to /private/var. Those ancestors are
    // already accounted for by the `dunce::canonicalize(beads_dir)` that
    // produced `canonical_beads`, and are outside this check's remit anyway.
    for ancestor in path.ancestors() {
        // Locate `ancestor` in the real tree by canonicalizing *its parent*
        // and re-appending its own name -- not by canonicalizing `ancestor`
        // itself. If `ancestor` is the symlink under test, canonicalizing it
        // directly would jump straight to its target and we'd never get to
        // inspect the symlink itself; going through the parent resolves
        // every ancestor *above* `ancestor` while leaving `ancestor`'s own
        // symlink-ness (checked below) untouched.
        let Some(parent) = ancestor.parent() else {
            break; // reached the filesystem root
        };
        let Ok(canonical_parent) = dunce::canonicalize(parent) else {
            continue; // parent doesn't exist yet; nothing to check here
        };
        let Some(name) = ancestor.file_name() else {
            continue;
        };
        let real_location = canonical_parent.join(name);

        if real_location == *canonical_beads || canonical_beads.starts_with(&real_location) {
            // At or above the beads directory itself: already accounted for
            // by `dunce::canonicalize(beads_dir)`, and outside this check's
            // remit regardless. Nothing shallower can be in scope either.
            break;
        }

        if !real_location.starts_with(canonical_beads) {
            // `ancestor`'s real location isn't related to canonical_beads at
            // all, which means a *shallower* ancestor's symlink is what
            // actually carried us outside. Keep walking up so that ancestor
            // gets checked (and reported) directly, instead of blaming the
            // leaf for a symlink further up the chain.
            continue;
        }

        // Strictly below canonical_beads: in scope for this check.
        ...
        if let Some(rejection) =
            classify_symlink_ancestor(ancestor, canonical_beads, std::fs::read_link(ancestor))
        {
...
/// Decides whether a confirmed symlink ancestor is a rejection, given the
/// result of reading its target.
///
/// Split out from the ancestor walk above so this branch is reachable from a
/// test. The `read_link` failure it handles is a TOCTOU window --
/// `symlink_metadata` confirmed a symlink and `read_link` then failed, so the
/// link changed or became unreadable between the two syscalls -- which cannot
/// be provoked deterministically through the filesystem: on Unix the two calls
/// resolve the same path the same way, and anything that breaks the second
/// (an unsearchable parent, a removed link) breaks the first as well. Passing
/// the `read_link` result in as an argument is what makes the failure branch
/// testable at all; see `unreadable_symlink_target_is_a_rejection`. Only the
/// one-line wiring at the call site is left uncovered.
fn classify_symlink_ancestor(
    ancestor: &Path,
    canonical_beads: &Path,
    link_target: std::io::Result<PathBuf>,
) -> Option<PathValidation> {
    // Fail CLOSED. This previously fell back to `target = ancestor`, which
    // trivially satisfies the `starts_with` below and so reported *no* escape
    // -- a path-escape check answering "safe" precisely when it has no
    // evidence either way. We cannot show the link escapes, only that we
    // cannot show it does not, so it is rejected under its own variant rather
    // than reported as a confirmed escape.
    let target = match link_target {
        Ok(target) => resolve_symlink_target_for_validation(ancestor, &target),
        Err(error) => {
            return Some(PathValidation::SymlinkTargetUnreadable {
    ...
    Some(PathValidation::SymlinkEscape {
        path: ancestor.to_path_buf(),
        target,
    })
}
...
mod tests {
    ...
    #[test]
    fn accepts_beads_dir_under_a_symlinked_ancestor() {
        // macOS $TMPDIR is /var/folders/…, and /var is a symlink to /private/var.
        // A .beads directory sitting under a symlinked ancestor is ordinary, not
        // an escape: the ancestor resolves to a path that *contains* the beads
        // dir, so it can never satisfy starts_with(canonical_beads).
        ...
        let beads_dir = link.join(".beads");
        std::fs::create_dir_all(&beads_dir).expect("beads dir");
        let target = beads_dir.join("issues.jsonl");
        std::fs::write(&target, "").expect("write");

        let result = validate_sync_path(&target, &beads_dir);
        assert!(
            result.is_allowed(),
            "a .beads dir under a symlinked ancestor must be allowed, got: {result:?}"
        );
    }

    #[test]
    fn still_rejects_a_symlink_inside_beads_pointing_out() {
        // The escape this check actually exists to stop: an *intermediate*
        // directory inside .beads that symlinks out, e.g. `.beads/sub ->
        // /tmp/evil`. A leaf-is-the-symlink variant is also rejected, but by
        // the independent is_symlink() check on the final path component in
        // validate_sync_path -- that check runs regardless of what
        // symlink_escape_for_existing_ancestor decides, so it would pass
        // even with the ancestor walk stubbed out to always return `None`.
        // This test must exercise the walk itself, or it proves nothing.
        let workspace = TempDir::new().expect("temp dir");
        let outside = TempDir::new().expect("outside");
        let outside_file = outside.path().join("issues.jsonl");
        std::fs::write(&outside_file, "").expect("write outside");

        let beads_dir = workspace.path().join(".beads");
        std::fs::create_dir_all(&beads_dir).expect("beads dir");
        let sub = beads_dir.join("sub");
        std::os::unix::fs::symlink(outside.path(), &sub).expect("symlink");

        let escaping = sub.join("issues.jsonl");
        let result = validate_sync_path(&escaping, &beads_dir);
        assert!(
            matches!(result, PathValidation::SymlinkEscape { .. }),
            "a symlinked intermediate directory inside .beads pointing outside must still be rejected as a SymlinkEscape, got: {result:?}"
        );
    }

    /// bds-04l.10. The branch under test is a TOCTOU window --
    /// `symlink_metadata` confirmed a symlink and `read_link` then failed --
    /// which cannot be produced through the filesystem, so the `read_link`
    /// result is injected instead. Before the fix this path fell back to
    /// `target = ancestor`, which trivially satisfies `starts_with` and so
    /// returned `None`: the guard reported "no escape" on no evidence either
    /// way, which is the wrong direction for a path-escape check.
    #[test]
    fn unreadable_symlink_target_is_a_rejection() {
        let (_temp, beads_dir) = setup_test_beads_dir();
        let canonical = dunce::canonicalize(&beads_dir).expect("canonicalize beads dir");
        let ancestor = canonical.join("sub");

        let result = classify_symlink_ancestor(
            &ancestor,
            &canonical,
        ...
        assert!(
            matches!(result, Some(PathValidation::SymlinkTargetUnreadable { .. })),
            "a symlink whose target cannot be read must fail closed, got: {result:?}"
        );
    }

    /// Companion to the above: the two readable outcomes must keep their
    /// previous classification, so the fail-closed change is shown to affect
    /// only the error path and not to have turned every symlink into a
    /// rejection.
    #[test]
    fn readable_symlink_target_inside_beads_is_not_a_rejection() {
        let (_temp, beads_dir) = setup_test_beads_dir();
        let canonical = dunce::canonicalize(&beads_dir).expect("canonicalize beads dir");
        let inside = canonical.join("real");
        std::fs::create_dir_all(&inside).expect("create inside target");
        let ancestor = canonical.join("sub");

        let result = classify_symlink_ancestor(&ancestor, &canonical, Ok(inside));

        assert!(
            result.is_none(),
            "a link resolving inside .beads is not an escape, got: {result:?}"
        );
    }

    #[test]
    fn readable_symlink_target_outside_beads_is_an_escape() {
        let (_temp, beads_dir) = setup_test_beads_dir();
        ...
        let result =
            classify_symlink_ancestor(&ancestor, &canonical, Ok(outside.path().to_path_buf()));

        assert!(
            matches!(result, Some(PathValidation::SymlinkEscape { .. })),
            "a link resolving outside .beads is an escape, got: {result:?}"
        );
    }
