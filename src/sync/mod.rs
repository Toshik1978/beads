...
use crate::storage::conn::{DbError, SqliteValue};
...
/// Stream every issue record in `input_path` through `handle_issue`.
...
";
    ...
    #[test]
    fn test_preflight_import_prefix_rejects_ids_with_extra_segments() {
        // `br` no longer generates, nor tolerates, an ID carrying a
        // human-readable segment between the prefix and the hash. Such an ID
        // is a prefix mismatch like any other. This repository's own
        // `issues.jsonl` contains none, so without this test nothing would
        // notice the tolerance being reintroduced.
        let temp = TempDir::new().unwrap();
        let beads_dir = temp.path().join(".beads");
        std::fs::create_dir_all(&beads_dir).unwrap();
        let jsonl_path = beads_dir.join("issues.jsonl");

        let issue = make_test_issue("bd-survey-my-thing-abc123", "Extra ID segment");
        let json = serde_json::to_string(&issue).unwrap();
        std::fs::write(&jsonl_path, format!("{json}\n")).unwrap();
        ...
        let result = preflight_import(&jsonl_path, &config, Some("bd")).unwrap();
        let prefix_check = result
            .checks
            .iter()
            .find(|c| c.name == "prefix_match")
            .expect("prefix_match check should be present");
        assert_eq!(
            prefix_check.status,
            PreflightCheckStatus::Fail,
            "an ID with a segment between the prefix and the hash is a mismatch"
        );

        // And the import itself refuses it, which is the end state: a
        // workspace that still holds such IDs must be rewritten before it can
        // be imported.
        let mut storage = SqliteStorage::open_memory().unwrap();
        ...
        .unwrap_err();
        assert!(err.to_string().contains("Prefix mismatch"));
        assert!(err.to_string().contains("bd-survey-my-thing-abc123"));
    }

    #[test]
    fn test_id_matches_expected_prefix_keeps_non_delimited_supersets_out() {
        assert!(id_matches_expected_prefix("bd-abc123", "bd"));
        assert!(id_matches_expected_prefix("bd-abc123", "bd-"));
